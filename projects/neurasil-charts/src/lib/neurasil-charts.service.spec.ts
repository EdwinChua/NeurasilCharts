...
describe('NeurasilChartsService', () => {
    ...
    function buildChart(chartType: NEURASIL_CHART_TYPE, chartData: Record<string, any[]>, overrides: Partial<{
        useAltAxis: boolean;
        title: string;
        ...
        return service.chartObjectBuilder(chartType, chartData ?? o.data, overrides.useAltAxis ?? false, overrides.title ?? '', overrides.yAxisLabelText ?? '', overrides.yAxisLabelText_Alt ?? '', overrides.xAxisLabelText ?? '', o._cornerstone, overrides.swapLabelsAndDatasets ?? false, o._formatObject, overrides.useLogScale ?? false, overrides.colorPalette ?? null, 0.9, 0.5, 1, 1);
    }
    ...
    describe('chartObjectBuilder', () => {
        ...
        it('should include a plugins.title when a title is provided', () => {
            ...
            expect(result.options.plugins.title.display).toBe(true);
        ...
        it('should stack x and y axes for STACKED chart type', () => {
            ...
            expect(result.options.scales.x.stacked).toBe(true);
            expect(result.options.scales.yAxis.stacked).toBe(true);
        });
        ...
        it('should add a secondary yAxis_alt for BAR_LINE when useAltAxis is allowed', () => {
            ...
            const result = service.chartObjectBuilder(NEURASIL_CHART_TYPE.BAR_LINE, o.data, true, '', '', 'Right label', '', o._cornerstone, false, o._formatObject, false, null, 0.9, 0.5, 1, 1);
    ...
    describe('performParetoAnalysis', () => {
        ...
        it('should fill the 80% line dataset with 80 for every data point', () => {
            ...
            expect(line.data.every((v: number) => v === 80)).toBe(true);
        });
